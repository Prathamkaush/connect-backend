...
export class PaymentsController {
  ...
  @Post('create-order') create(@CurrentUser() user: AuthUser, @Body() dto: CreateOrderDto) { return this.payments.createOrder(user.id, dto); }
  @Post('verify') verify(@CurrentUser() user: AuthUser, @Body() dto: VerifyPaymentDto) { return this.payments.verify(user.id, dto); }
  @Get() list(@CurrentUser() user: AuthUser) { return this.payments.list(user.id); }
