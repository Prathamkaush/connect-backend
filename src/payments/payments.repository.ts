...
export class PaymentsRepository {
  ...
  listOwned(userId: string, query: PaymentHistoryDto = { limit: 5 }) {
    return this.prisma.payment.findMany({
      where: { userId, ...(query.before ? { OR: [
        { createdAt: { lt: new Date(query.before) } },
        ...(query.beforeId ? [{ createdAt: new Date(query.before), id: { lt: query.beforeId } }] : []),
      ] } : {}) },
      take: query.limit, orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
      include: { plan: { select: { name: true } }, invoice: true },
    });
  }
